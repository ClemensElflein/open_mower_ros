...
void MowingBehavior::reset() {
  ...
  currentMowingAngleIncrementSum = std::fmod(
      currentMowingAngleIncrementSum + getConfig().mow_angle_increment, 360);
  checkpoint();
...
bool MowingBehavior::create_mowing_plan(int area_index) {
  ROS_INFO_STREAM(
      "MowingBehavior: Creating mowing plan for area: " << area_index);
  ...
  double mow_angle_offset = std::fmod(
      getConfig().mow_angle_offset + currentMowingAngleIncrementSum + 180, 360);
  if (mow_angle_offset < 0) mow_angle_offset += 360;
  mow_angle_offset -= 180;
  ROS_INFO_STREAM(
      "MowingBehavior: mowing angle offset (deg): " << mow_angle_offset);
  if (config.mow_angle_offset_is_absolute) {
    ...
    ROS_INFO_STREAM(
        "MowingBehavior: Auto-detected mowing angle + mowing angle offset: "
        << angle);
  }
  ...
  for (const auto &path : currentMowingPaths) {
    for (const auto &pose_stamped : path.path.poses) {
      hash.Update(reinterpret_cast<const byte *>(&pose_stamped.pose),
                  sizeof(geometry_msgs::Pose));
    }
  }
  ...
  if (mowingPlanDigest == currentMowingPlanDigest) {
    ROS_INFO_STREAM("MowingBehavior: Advancing to checkpoint, path: "
                    << currentMowingPath
                    << " index: " << currentMowingPathIndex);
  } else {
    ROS_INFO_STREAM("MowingBehavior: Ignoring checkpoint for plan ("
                    << currentMowingPlanDigest << ") current mowing plan is ("
                    << mowingPlanDigest << ")");
...
int getCurrentMowPathIndex() {
  ...
  if (pathProgressClient.call(progressSrv)) {
    ...
    ROS_ERROR(
        "MowingBehavior: getMowIndex() - Error getting progress from FTC "
        "planner");
  }
  return (currentIndex);
}
...
void printNavState(int state) {
  switch (state) {
    ...
    case actionlib::SimpleClientGoalState::ABORTED:
      ROS_INFO(">>> State: Aborted <<<");
      break;
    case actionlib::SimpleClientGoalState::SUCCEEDED:
      ROS_INFO(">>> State: Succeeded <<<");
      break;
    case actionlib::SimpleClientGoalState::LOST:
      ROS_INFO(">>> State: Lost <<<");
      break;
    default:
      ROS_INFO(">>> State: Unknown Hu ? <<<");
      break;
  }
}
...
bool MowingBehavior::execute_mowing_plan() {
  ...
  while (currentMowingPath < currentMowingPaths.size() && ros::ok() &&
         !aborted) {
    ////////////////////////////////////////////////
    // PAUSE HANDLING
    ////////////////////////////////////////////////
    if (requested_pause_flag) {  // pause was requested
      ...
      while (requested_pause_flag)  // while emergency and/or manual pause not
                                    // asked to continue, we wait
      {
        ...
        ROS_INFO_STREAM_THROTTLE(
            30, "MowingBehavior: PAUSED (" << pause_reason << ")");
        ros::Rate r(1.0);
        r.sleep();
      }
    ...
    if (paused) {
      ...
      {
        ROS_INFO_STREAM("MowingBehavior: PAUSED ("
                        << (ros::Time::now() - paused_time).toSec()
                        << "s) (waiting for GPS)");
        ros::Rate r(1.0);
        r.sleep();
      }
    ...
    ROS_INFO_STREAM("MowingBehavior: Path segment length: "
                    << path.path.poses.size() << " poses.");
    ...
    //
    // * we have n attempts, if we fail we go to pause() mode because most
    // likely it was GPS problems that
    //   prevented us from reaching the inital pose
    // * after n attempts, we fail the mow area and skip to the next one
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
      ROS_INFO_STREAM(
          "MowingBehavior: (FIRST POINT)  Moving to path segment starting "
          "point");
      if (path.is_outline && getConfig().add_fake_obstacle) {
        mower_map::SetNavPointSrv set_nav_point_srv;
        set_nav_point_srv.request.nav_pose =
            path.path.poses[currentMowingPathIndex].pose;
      ...
      while (ros::ok()) {
        ...
        if (current_status.state_ == actionlib::SimpleClientGoalState::ACTIVE ||
          ...
          if (skip_area) {
            ROS_INFO_STREAM(
                "MowingBehavior: (FIRST POINT) SKIP AREA was requested.");
          ...
          if (aborted) {
            ROS_INFO_STREAM(
                "MowingBehavior: (FIRST POINT) ABORT was requested - stopping "
                "path execution.");
            mbfClientExePath->cancelAllGoals();
            mowerEnabled = false;
            return false;
          ...
          if (requested_pause_flag) {
            ROS_INFO_STREAM(
                "MowingBehavior: (FIRST POINT) PAUSE was requested - stopping "
                "path execution.");
            mbfClientExePath->cancelAllGoals();
            mowerEnabled = false;
            return false;
          ...
          ROS_INFO_STREAM(
              "MowingBehavior: (FIRST POINT)  Got status "
              << current_status.state_
      ...
      if (current_status.state_ !=
          actionlib::SimpleClientGoalState::SUCCEEDED) {
        // we cannot reach the start point
        ROS_ERROR_STREAM(
            "MowingBehavior: (FIRST POINT) - Could not reach goal (first "
            "point). Planner Status was: "
            << current_status.state_);
        ...
        if (first_point_attempt_counter < config.max_first_point_attempts) {
          ...
          // We failed to reach the first point in the mow path by simply
          // repeating the drive to process So now we will trim the path by
          // removing the first pose
          if (first_point_trim_counter < config.max_first_point_trim_attempts) {
            // We try now to remove the first point so the 2nd, 3rd etc point
            // becomes our target mow path points are offset by 10cm
            ROS_WARN_STREAM(
                "MowingBehavior: (FIRST POINT) - Attempt "
            ...
            first_point_trim_counter++;
            first_point_attempt_counter =
                0;  // give it another <config.max_first_point_attempts>
                    // attempts
            paused = true;
            update_actions();
          } else {
            // Unable to reach the start of the mow path (we tried multiple
            // attempts for the same point, and we skipped points which also
            // didnt work, time to give up)
            ROS_ERROR_STREAM(
                "MowingBehavior: (FIRST POINT) Max retries reached, we are "
                "unable to reach any of the first points - aborting at index: "
                << currentMowingPathIndex << " path: " << currentMowingPath
      ...
      // we have reached the start pose of the mow area, reset error handling
      // values
    ...
    // successfully or trim it if we were aborted
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
      // enable mower (only when we reach the start not on the way to mowing
      // already)
      mowerEnabled = true;
      ...
      exePath.poses = std::vector<geometry_msgs::PoseStamped>(
          path.path.poses.begin() + currentMowingPathIndex,
          path.path.poses.end());
      ...
      ROS_INFO_STREAM(
          "MowingBehavior: (MOW) First point reached - Executing mow path with "
          << path.path.poses.size() << " poses, from index "
          << exePathStartIndex);
      ...
      while (ros::ok()) {
        ...
        if (current_status.state_ == actionlib::SimpleClientGoalState::ACTIVE ||
          ...
          if (aborted) {
            ROS_INFO_STREAM(
                "MowingBehavior: (MOW) ABORT was requested - stopping path "
                "execution.");
            mbfClientExePath->cancelAllGoals();
            mowerEnabled = false;
            break;  // Trim path
          ...
          if (requested_pause_flag) {
            ROS_INFO_STREAM(
                "MowingBehavior: (MOW) PAUSE was requested - stopping path "
                "execution.");
            mbfClientExePath->cancelAllGoals();
            mowerEnabled = false;
            break;  // Trim path
          ...
          if (current_status.state_ ==
            ...
            ROS_INFO_STREAM_THROTTLE(5, "MowingBehavior: (MOW) Progress: "
                                            << currentMowingPathIndex << "/"
                                            << path.path.poses.size());
            if (ros::Time::now() - last_checkpoint > ros::Duration(30.0))
              checkpoint();
          }
        } else {
          ROS_INFO_STREAM(
              "MowingBehavior: (MOW)  Got status "
              << current_status.state_
              << " from MBF/FTCPlanner -> Stopping path execution.");
          // we're done, break out of the loop
          break;
      ...
      if (current_status.state_ != actionlib::SimpleClientGoalState::PENDING &&
        ...
        ROS_INFO_STREAM(
            ">> MowingBehavior: (MOW) PlannerGetProgress "
            "currentMowingPathIndex = "
        ...
        // error, drop the path segment
        /* TODO: we can not trust the SUCCEEDED state because the planner
           sometimes says suceeded with the currentIndex far from the size of
           the poses ! (BUG in planner ?)
            instead we trust only the currentIndex vs. poses.size() */
        if (currentMowingPathIndex >= path.path.poses.size() ||
            (path.path.poses.size() - currentMowingPathIndex) <
                5)  // fully mowed the path ?
        {
          ROS_INFO_STREAM(
              "MowingBehavior: (MOW) Mow path finished, skipping to next mow "
              "path.");
          currentMowingPath++;
          currentMowingPathIndex = 0;
          ...
          // TODO: we should figure out the likely reason for our failure to
          // complete the path if GPS -> PAUSE if something else -> Recovery
          // Behaviour ?

          // currentMowingPathIndex might be 0 if we never consumed one of the
          // points, we advance at least 1 point
          if (currentMowingPathIndex == 0) currentMowingPathIndex++;
          if (!requested_pause_flag) {
            ROS_INFO_STREAM("MowingBehavior: (MOW) PAUSED due to MBF Error at "
                            << currentMowingPathIndex);
            paused = true;
            update_actions();
          }
...
void MowingBehavior::command_start() {
  ...
  if (shared_state->active_semiautomatic_task &&
    ...
    // We are in semiautomatic task and paused, user wants to resume, so store
    // that immediately. This way, once we are docked the mower will continue as
    // soon as all other conditions are g2g
...
uint8_t MowingBehavior::get_sub_state() { return 0; }
uint8_t MowingBehavior::get_state() {
  return mower_msgs::HighLevelStatus::HIGH_LEVEL_STATE_AUTONOMOUS;
}
int16_t MowingBehavior::get_current_area() { return currentMowingArea; }
int16_t MowingBehavior::get_current_path() { return currentMowingPath; }
    ...
    for (rosbag::MessageInstance const m : view) {
      ...
      if (cp) {
        ROS_INFO_STREAM(
            ...
            << " area: " << cp->currentMowingArea << " path: "
            << cp->currentMowingPath << " index: " << cp->currentMowingPathIndex
