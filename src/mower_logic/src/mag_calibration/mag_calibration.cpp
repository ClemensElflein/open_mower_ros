...
void magReceived(const sensor_msgs::MagneticField::ConstPtr &msg) {
  ...
  filteredX =
      filterFactor * filteredX + (1.0 - filterFactor) * msg->magnetic_field.x;
  filteredY =
      filterFactor * filteredY + (1.0 - filterFactor) * msg->magnetic_field.y;
  filteredZ =
      filterFactor * filteredZ + (1.0 - filterFactor) * msg->magnetic_field.z;
  ...
  ROS_INFO_STREAM_THROTTLE(1,
                           "\n"
                               ...
                               << "; x_bias = " << (minX + maxX) / 2.0 << "\n"
                               << "min_y = " << minY << "; max_y = " << maxY
                               << "; y_bias = " << (minY + maxY) / 2.0 << "\n"
                               << "min_z = " << minZ << "; max_z = " << maxZ
                               << "; z_bias = " << (minZ + maxZ) / 2.0 << "\n");
}
...
int main(int argc, char **argv) {
  ...
  std::cout
      << "Rotate the mower around all axes and press enter once you are done.";
  sleep(2);
