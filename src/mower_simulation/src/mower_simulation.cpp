...
dynamic_reconfigure::Server<mower_simulation::MowerSimulationConfig>
    *reconfig_server;
...
bool setGpsState(xbot_positioning::GPSControlSrvRequest &req,
...
void publishStatus(const ros::TimerEvent &timer_event) {
  ...
  if (config.mower_error) {
    fake_mow_status.mow_esc_status.status =
  ...
  is_docked =
      has_dock && sqrt(pow(pose.pose.pose.position.x - dockX, 2) +
                       pow(pose.pose.pose.position.y - dockY, 2)) < 0.25;

  if (last_is_docked != is_docked) {
    ROS_INFO_STREAM(
        "simulation is inside dock: " << (is_docked ? "YES" : "NO"));
  }
  ...
  if (config.wheels_stalled) {
    fake_mow_status.left_esc_status.status =
        mower_msgs::ESCStatus ::ESC_STATUS_STALLED;
    fake_mow_status.right_esc_status.status =
        mower_msgs::ESCStatus ::ESC_STATUS_STALLED;
  } else {
    fake_mow_status.left_esc_status.status =
...
bool setPose(xbot_positioning::SetPoseSrvRequest &req,
...
int main(int argc, char **argv) {
  ...
  reconfig_server =
      new dynamic_reconfigure::Server<mower_simulation::MowerSimulationConfig>(
          paramNh);
  ...
  if (!docking_point_client.waitForExistence(ros::Duration(60.0, 0.0))) {
      ...
      docking_pose_stamped.pose.pose =
  get_docking_point_srv.response.docking_pose;
  ...
  */
  ...
  pose_pub =
      n.advertise<xbot_msgs::AbsolutePose>("/xbot_positioning/xb_pose", 1);
  ...
  ros::Subscriber odom_sub = n.subscribe(
      "/mower/odom", 0, odomReceived, ros::TransportHints().tcpNoDelay(true));
  ros::ServiceServer mow_service =
      n.advertiseService("mower_service/mow_enabled", setMowEnabled);
  ros::ServiceServer gps_service =
      n.advertiseService("xbot_positioning/set_gps_state", setGpsState);
  ...
  ros::ServiceServer pose_service =
      n.advertiseService("xbot_positioning/set_robot_pose", setPose);
